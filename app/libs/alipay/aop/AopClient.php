...
class AopClient {
	...
	*/
	...
	public function checkSignAndalipayalipayalipayDecrypt($params, $rsaPublicKeyPem, $rsaPrivateKeyPem, $isCheckSign, $isalipayalipayalipayDecrypt) {
		...
		if ($isalipayalipayalipayDecrypt) {
			return $this->rsaalipayalipayalipayDecrypt($bizContent, $rsaPrivateKeyPem, $charset);
		}
	...
	public function rsaalipayalipayalipayDecrypt($data, $rsaPrivateKeyPem, $charset) {
		//读取私钥文件
		...
		foreach ($decodes as $n => $decode) {
			if (!openssl_private_alipayalipayalipayDecrypt($decode, $dcyCont, $res)) {
				echo "<br/>" . openssl_error_string() . "<br/>";
			}
			$strnull .= $dcyCont;
		}
	...
	private function encryptJSONSignSource($request, $responseContent) {
		...
		$bizContent = alipayalipayalipayDecrypt($parsetItem->encryptContent, $this->encryptKey);
